...
function [output] = n_fold_CV_SVM(X,Y, n, sw, data)
%X: Features
%Y: Class Variable
...
for i = 1 : n
    ...
    if sw == 0
        SVM{i}.Model = fitcsvm(Xtt, yt);
        
    elseif sw==1
        SVM{i}.Model = fitcsvm(Xtt, yt, 'KernelFunction','rbf');
    end
    [label,  p(b{i},1:2)] = predict(SVM{i}.Model, Xst);
...
X_trans = X*T_best;
SVM_best_model =  SVM{best_ind}.Model;
[~,  p_train(:,1:2)] = predict(SVM{i}.Model, X_trans);
...
output.SVM_best_model = SVM_best_model;
